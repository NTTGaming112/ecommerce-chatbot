  ...
  return lines.map((line, i) => {
    ...
    if (/^\s*[-•]\s/.test(line)) {
      ...
      while ((bm = bRe.exec(bulletContent)) !== null) {
        if (bm.index > bi)
          bParts.push(
            <span key={"b" + bi}>{bulletContent.slice(bi, bm.index)}</span>,
          );
        bParts.push(
          <strong key={"bb" + bm.index} className="font-semibold text-white">
            {bm[1]}
          </strong>,
        );
        bi = bRe.lastIndex;
      }
      if (bi < bulletContent.length)
        bParts.push(<span key={"be" + bi}>{bulletContent.slice(bi)}</span>);
      return (
    ...
    if (/^\s*\d+\.\s/.test(line)) {
      ...
      while ((cm = cbRe.exec(content)) !== null) {
        if (cm.index > ci)
          contentParts.push(
            <span key={"c" + ci}>{content.slice(ci, cm.index)}</span>,
          );
        contentParts.push(
          <strong key={"cb" + cm.index} className="font-semibold text-white">
            {cm[1]}
          </strong>,
        );
        ci = cbRe.lastIndex;
      ...
      if (ci < content.length)
        contentParts.push(<span key={"ce" + ci}>{content.slice(ci)}</span>);
      return (
        <div key={i} className="flex gap-2 ml-2">
...
function LoginScreen({
  ...
  onLogin: (customerId: string, name: string) => void;
}) {
...
function ChatApp({
  ...
  const [messages, setMessages] = useState<
    { role: "user" | "bot"; content: string; meta?: string }[]
  >([]);
  ...
  const handleSend = async (message: string) => {
    ...
    try {
      ...
      const agentTrace =
        Array.isArray(data.actions_taken) && data.actions_taken.length > 0
          ? data.actions_taken.join(", ")
          : "No agent trace";
      const meta = `Intent: ${data.intent} | Confidence: ${(data.confidence * 100).toFixed(0)}% | Agents: ${agentTrace}`;
      setMessages((prev) => [
        ...prev,
        { role: "bot", content: data.answer, meta },
      ]);
    } catch (error: any) {
      setMessages((prev) => [
        ...prev,
      ...
      <div className="w-full md:w-80 bg-slate-800 border-b md:border-b-0 md:border-r border-slate-700 p-6 flex flex-col shrink-0">
        ...
        <div className="flex-1">
          ...
          >
            ...
            />
            <svg
              className="w-8 h-8 text-slate-500 mx-auto mb-2 group-hover:text-blue-400 transition-colors"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
